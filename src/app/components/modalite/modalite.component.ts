...
export class ModaliteComponent implements OnInit {
  ...
  versement: number;
  contrat: Contrat;
  addContrat: { client: any; echeanciers: any; facture: any; articles: any; };
  ...
  otherContrat() {
    ...
    console.log(this.addContrat);

    this.contratService.addOtherContrat(this.contrat).subscribe(data => {
      alert(' Vente enregistrée avec succé');
      //set iitem Contrat in localStorage
      localStorage.setItem('currentContrat', JSON.stringify(data));
